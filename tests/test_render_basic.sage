...
# Ground plane near the start position
let gz = 0
while gz < 32:
    let gx = 0
    while gx < 32:
        ...
        if gz % 4 == 0 and gx % 4 == 0:
            set_voxel(voxel, gx + 18, 11, gz + 6, 2)  # dirt pillars for depth
        gx = gx + 1
    gz = gz + 1

# Floating water and lava platforms for contrast
let wz = 0
while wz < 16:
    let wx = 20
    while wx < 30:
...
print "✓ Terrain generated with ground plane and platforms"
    ...
    while di < len(visible_chunks):
        ...
        if draw != nil and dict_has(draw, "gpu_mesh"):
            ...
            if mesh != nil:
                ...
                # Choose a debug color by block type
                let block_id = 0
                ...
                if rendered_chunks == 1 and frame_count % 60 == 0:
                    print "Drew first chunk mesh with block_id=" + str(block_id)
        di = di + 1
