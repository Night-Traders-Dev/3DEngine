...
let pp = {}
pp["bloom_threshold"] = 0.72
pp["bloom_soft_knee"] = 0.28
pp["highlight_saturation"] = 1.1
pp["bloom_width"] = 640.0
pp["bloom_height"] = 360.0
pp["bloom_radius"] = 1.0
pp["exposure"] = 1.14
pp["bloom_intensity"] = 0.34
pp["tonemap_mode"] = TONEMAP_ACES
pp["gamma"] = 2.2
pp["contrast"] = 1.08
pp["saturation"] = 1.16
pp["warmth"] = 0.08
pp["vignette_strength"] = 0.10

let extract = build_bloom_extract_push_data(pp)
check("extract push has 4 floats", len(extract) == 4)
check("extract push stores threshold and knee", extract[0] == 0.72 and extract[1] == 0.28)
...
check("tonemap push stores exposure and bloom", tonemap[0] == 1.14 and tonemap[1] == 0.34)
check("tonemap push stores grading params", tonemap[4] == 1.08 and tonemap[7] == 0.10)
...
check("editor preset is milder than shaderpack", preset["bloom_intensity"] < 0.34 and preset["exposure"] < 1.14)
