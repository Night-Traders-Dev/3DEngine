...
proc _sage_literal(value):
    if value == nil:
        return "nil"
    let t = type(value)
    if t == "bool":
        return _bool_lit(value)
    if t == "number":
        return _fmt(value)
    if t == "string":
        let q = chr(34)
        return q + _escape_string(value) + q
    if t == "array":
        let out = "["
        let i = 0
        while i < len(value):
    ...
    if t == "dict":
        ...
        while i < len(keys):
            if i > 0:
                out = out + ", "
            out = out + _sage_literal(keys[i]) + ": " + _sage_literal(value[keys[i]])
            i = i + 1
        return out + "}"
    return "nil"
...
proc generate_game_script(world, scene_name, settings):
    ...
    let has_voxel_worlds = len(query(world, ["voxel_world"])) > 0
    ...
    if has_voxel_worlds:
        push(L, "from voxel_world import voxel_world_from_sage, voxel_visible_draws")
    push(L, "from sky import create_sky, sky_preset_day, init_sky_gpu, draw_sky")
    push(L, "from shadow_map import create_shadow_renderer, compute_light_vp_stable, primary_shadow_light")
    ...
    push(L, "        return false")
    push(L, "    let light_vp = compute_light_vp_stable(shadow_light[" + q + "direction" + q + "], focus_point, 45.0, shadow_renderer[" + q + "resolution" + q + "] + 0.0)")
    ...
    push(L, "        ri = ri + 1")
    if has_voxel_worlds:
        push(L, "    let voxel_rl = query(world, [" + q + "transform" + q + ", " + q + "voxel_world" + q + "])")
        push(L, "    let vi = 0")
        push(L, "    while vi < len(voxel_rl):")
        push(L, "        let eid = voxel_rl[vi]")
        push(L, "        let t = get_component(world, eid, " + q + "transform" + q + ")")
        push(L, "        let vw = get_component(world, eid, " + q + "voxel_world" + q + ")")
        push(L, "        let model = transform_to_matrix(t)")
        push(L, "        let draws = voxel_visible_draws(vw, focus_point[0] - t[" + q + "position" + q + "][0], focus_point[1] - t[" + q + "position" + q + "][1], focus_point[2] - t[" + q + "position" + q + "][2], 2)")
        push(L, "        let di = 0")
    ...
    while ei < len(all_ents):
        ...
        if has_component(world, eid, "voxel_world"):
            from voxel_world import voxel_world_to_sage
            let voxel_comp = get_component(world, eid, "voxel_world")
            push(L, "let " + v + "_voxel = voxel_world_from_sage(" + _sage_literal(voxel_world_to_sage(voxel_comp)) + ")")
            push(L, "add_component(world, " + v + ", " + q + "voxel_world" + q + ", " + v + "_voxel)")
    ...
    if has_voxel_worlds:
        push(L, "    let voxel_rl = query(world, [" + q + "transform" + q + ", " + q + "voxel_world" + q + "])")
        push(L, "    let vri = 0")
        push(L, "    while vri < len(voxel_rl):")
        push(L, "        let eid = voxel_rl[vri]")
        push(L, "        let t = get_component(world, eid, " + q + "transform" + q + ")")
        push(L, "        let vw = get_component(world, eid, " + q + "voxel_world" + q + ")")
        push(L, "        let model = transform_to_matrix(t)")
        push(L, "        let voxel_mvp = mat4_mul(vp, model)")
        push(L, "        let draws = voxel_visible_draws(vw, player[" + q + "position" + q + "][0] - t[" + q + "position" + q + "][0], player[" + q + "position" + q + "][1] - t[" + q + "position" + q + "][1], player[" + q + "position" + q + "][2] - t[" + q + "position" + q + "][2], 2)")
        push(L, "        let di = 0")
        push(L, "        while di < len(draws):")
